...
for i in range(Tn):
	A0list[i][1:-1,1:-1] = mass+tau*stiff
	A0list[i][0,0] =1; A0list[i][N-1,N-1] = 1
...
precon = temp3+temp1
precon[:N,-N:] =1.*A1
precon = precon.getH()*precon
PP = sp.linalg.sqrtm(precon.todense())
precon = ssp.csr_matrix(PP)
# Construct a linear operator that computes P^-1 * x.
import scipy.sparse.linalg as spla
M_x = lambda x: spla.spsolve(precon, x)
P = spla.LinearOperator((N*Tn, N*Tn), M_x)
'''
...
#CC = BB*PP
precon = PP.transpose()*PP
P = sp.linalg.sqrtm(precon.todense())
